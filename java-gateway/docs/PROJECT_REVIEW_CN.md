# ModelPort 多模型智能网关：架构复审与面试材料

## 1. 项目定位

ModelPort 是面向内部 AI 应用的模型访问网关，不是 Agent。它统一 OpenAI/Anthropic 协议、模型路由、认证、限流、预算、幂等、用量账本、Provider 健康状态和故障转移，使上层应用只依赖一个稳定端点。
...
ModelPort 不需要改造成 Agent。它的价值在确定性基础设施：协议兼容、策略路由、配额、账本、失败隔离和证据。LLM 不应参与每次网关路由判断，否则网关自身会引入额外成本、延迟和循环依赖。
